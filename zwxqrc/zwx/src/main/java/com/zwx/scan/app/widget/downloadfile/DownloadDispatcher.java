...
class DownloadDispatcher extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                LogUtils.v("Download initiated for " + request.getDownloadId());
    ...
    private void executeDownload(DownloadRequest request, String downloadUrl) {
        ...
        try {
            ...
            LogUtils.d(TAG, "Existing file mDownloadedCacheSize: " + mDownloadedCacheSize);
            ...
            LogUtils.v("Response code obtained for downloaded Id "
            ...
            switch (responseCode) {
                ...
                case HTTP_OK:
                    ...
                    if (readResponseHeaders(request, conn, responseCode) == 1) {
                        LogUtils.d(TAG, "Existing mDownloadedCacheSize: " + mDownloadedCacheSize);
                        LogUtils.d(TAG, "File mContentLength: " + mContentLength);
                        if (mDownloadedCacheSize == mContentLength) { // Mark as success, If end of stream already reached
                            updateDownloadComplete(request);
                            LogUtils.d(TAG, "Download Completed");
                        } else {
                ...
                case HTTP_TEMP_REDIRECT:
                    ...
                    while (mRedirectionCount < MAX_REDIRECTS && shouldAllowRedirects) {
                        mRedirectionCount++;
                        LogUtils.v(TAG, "Redirect for downloaded Id " + request.getDownloadId());
    ...
    private void transferData(DownloadRequest request, InputStream in, RandomAccessFile out) {
        ...
        LogUtils.v("Content Length: " + mContentLength + " for Download Id " + request.getDownloadId());
        for (; ; ) {
            if (request.isCancelled()) {
                LogUtils.v("Stopping the download as Download Request is cancelled for Downloaded Id " + request.getDownloadId());
                request.finish();
                updateDownloadFailed(request, DownloadManager.ERROR_DOWNLOAD_CANCELLED, "Download cancelled");
                return;
            }
    ...
    private int readResponseHeaders(DownloadRequest request, HttpURLConnection conn, int responseCode) {
        ...
        if (transferEncoding == null) {
            ...
            LogUtils.v("Ignoring Content-Length since Transfer-Encoding is also defined for Downloaded Id " + request.getDownloadId());
        }
    ...
    private void cleanupDestination(DownloadRequest request, boolean forceClean) {
        if (!request.isResumable() || forceClean) {
            LogUtils.d("cleanupDestination() deleting " + request.getDestinationURI().getPath());
            File destinationFile = new File(request.getDestinationURI().getPath());
            if (destinationFile.exists()) {
