...
public class GiveCouponFragment extends BaseFragment implements View.OnClickListener{
   ...
   /* public static Fragment getInstance(Coupon coupon,int curPosition) {
    ...
    public static Fragment getInstance(Coupon coupon,int curPosition) {
        GiveCouponFragment fragment = new GiveCouponFragment();
        fragment.coupon = coupon;
        fragment.curPosition = curPosition;
        return fragment;
    }
...
//                .bitmapTransform(roundedCorners)
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case  R.id.ll_delete:
                GiveCouponNewNextActivity activity = (GiveCouponNewNextActivity)getActivity();
...
//                    activity.campaignCouponList.clear();
                    activity.chanageLayout(activity.couponList);
                }

                if(activity.forwardCouponList != null && activity.forwardCouponList.size()>0){
                    for(int i = 0 ;i<activity.forwardCouponList.size();i++){
                        CampaignCoupon campaignCoupon = activity.forwardCouponList.get(i);
                        if(campaignCoupon != null){
                            Long couponId =campaignCoupon.getCouponId();
                            if(couponId != null){
                                if(couponId == coupon.getId()){
                                    activity.forwardCouponList.remove(i);
                                    i--;
                                }
                            }
    ...
    public  interface  ChanageGiveToLayoutLisenter{
