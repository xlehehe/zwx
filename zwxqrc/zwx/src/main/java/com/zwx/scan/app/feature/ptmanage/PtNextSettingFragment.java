 ...
 */
public class PtNextSettingFragment extends BaseFragment<PtContract.Presenter> implements PtContract.View,View.OnClickListener{
    ...
    private PtNextTwoFragment fragment =null;
    ...
    public PtNextSettingFragment() {
        // Required empty public constructor
    }

    public static PtNextSettingFragment getInstance(Coupon coupon, int curPosition, String name, NoScrollViewPager setViewPager) {
        PtNextSettingFragment fragment = new PtNextSettingFragment();
        fragment.coupon = coupon;
        fragment.curPosition = curPosition;
        fragment.setViewPager = setViewPager;
        fragment.ptName = name;
        return fragment;
    }
    ...
    protected void initView() {
        ...
        presenter.queryBrandAndStoreList(getActivity(),userId,true,false);
        ...
        fragment = new PtNextTwoFragment();


      /*  brandLogo = SPUtils.getInstance().getString("brandLogoPt");
        brandPostion = SPUtils.getInstance().getInt("brandPostionPt");

        storeIdCouponType = coupon.getStoreSelectType();
        if("商品设置".equals(ptName)){

            if(storeIdCouponType != null && !"".equals(storeIdCouponType)){
                storeIdCouponType = SPUtils.getInstance().getString("storeIdTypePt1");
            }

            storeIdArray= SPUtils.getInstance().getString("storeIdArrayPt1");
            storeNameArray= SPUtils.getInstance().getString("storeNameArrayPt1");
            ...
            if(storeIdCouponType != null && !"".equals(storeIdCouponType)){
                storeIdCouponType = SPUtils.getInstance().getString("storeIdTypePt2");
            }
            storeIdArray= SPUtils.getInstance().getString("storeIdArrayPt2");
            storeNameArray= SPUtils.getInstance().getString("storeNameArrayPt2");
            if("S".equals(PtNewActivity.compaignStatus)){
                storeIdArray = coupon.getStoreIds();
                storeNameArray = coupon.getStoreNames();
            }else if( "NEW".equals(PtNewActivity.compaignStatus)){
...
//        setViewPager.setLayoutParams(linearParams);
       ...
       /* if("A".equals(expireEndType)){
        ...
        if("A".equals(expireEndType)){
            ...
            campaignCoupon.setExpireStartDay(startDay!=null &&!"0".equals(startDay)?startDay:"0");
            campaignCoupon.setExpireEndDay(endDay!=null &&!"0".equals(endDay)?endDay:"1");
        ...
        edtDayDate.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if( getValidDay >99){
                    ...
                    getValidDay = 99;
                    ivRightAdd.setImageResource(R.drawable.ic_gray_add);
                }else {
                    ivRightAdd.setImageResource(R.drawable.ic_blue_added);
                }
                if(getValidDay>=1){
                    validDaysStr = getValidDay+"";
                    ivLeftSubtract.setImageResource(R.drawable.ic_blue_subtracted);
                    tvValidDays.setText(validDaysStr);
                }else {
                    validDaysStr = "当";
                    ivLeftSubtract.setImageResource(R.drawable.ic_gray_unsubtract);
                    tvValidDays.setText(validDaysStr);
                }
            ...
            for (int i=0;i< activity.campaignCouponList2.size();i++) {
                ...
                if (couponId != null && couponId2 != null) {
                    ...
                    if (couponId.equals(couponId2) &&  isSort.equals(isSort2)) {
                        ...
                        campaignCoupon.setStores(selectStoreList);
                        campaignCoupon.setExpireStartDay(startDay != null && !"".equals(startDay)?startDay:"0");
                        campaignCoupon.setExpireEndDay(endDay != null && !"".equals(endDay)?endDay:"1");
                        if(storeIdCouponType!=null && !"".equals(storeIdCouponType)){
                            if("A".equals(storeIdCouponType)||"D".equals(storeIdCouponType)||"J".equals(storeIdCouponType)){   //全部店铺  自营  加盟
                                campaignCoupon.setStoreStr("");
                        ...
                        if(material != null){
                            String background = material.getBackground();
                            String backgroundThumbnail = material.getBackgroundThumbnail();
                            campaignCoupon.setBackground(background);
                            campaignCoupon.setBackgroundThumbnail(backgroundThumbnail);
                        }
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_select_shop:  //开始时间
                ...
                if(storeList != null && storeList.size()>0){
                    ...
                    presenter.queryBrandAndStoreList(getActivity(),userId,false,false);
                }
